...
class Model
{
    ...
    /**
     * cherche les valeurs en fonction des paramêtre et peut les groups grâce a un group by
     * 
     * @param parametre Tableau avec en clé les colone est valeur les valeurs a faire correpondre
     * @param groupBy   Colone sur laquelle on veux appliqué le group by
     */
    ...
    /**
     * récupére toute les lignes de la table d'en face (a utilisé dans un contexte de relationship *-N)
     */
    public function hasMany(string $class, string $primaryKey, string $referenceKey)
    {
        ...
        $sql .= " from $thisTable AS a JOIN $joinTable AS b ON a.$primaryKey = b.$referenceKey WHERE a.$this->primaryKey = :id";

        try {
            $st2 = db()->prepare($sql);
            $st2->bindValue('id', self::$tempId, PDO::PARAM_STR);
            $st2->execute();
        } catch (PDOException $exception) {
            echo $exception->getMessage();
            die();
     ...
     /**
     * récupére toute une ligne de la table d'en face (a utilisé dans un contexte de relationship *-1)
     */
    public function belongsTo(string $class, string $foreignKey, string $localKey)
    {
        $thisClass = get_called_class();
        $thisTable = strtoupper($thisClass);
        $joinTable = strtoupper($class);
    ...
    /**
     * récupére toute les lignes de la table d'en face (a utilisé dans un contexte de relationship N-N)
     */
    ...
    /**
     * supprime une ligne de la table en question
     */
    ...
    public function save()
    {
        ...
        $test = array_diff_key(get_object_vars($this), ['primaryKey' => '']);
